"""
Test mutable MutableSet APIs for equivalence with set.

Tests in this module exercise only mutation operations (add, discard, and the mixin methods
built on top of them). Read-only API coverage lives in test_set.py, which runs those tests
against both Set and MutableSet implementations.
"""
